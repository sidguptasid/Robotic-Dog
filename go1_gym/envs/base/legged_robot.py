...
class LeggedRobot(BaseTask):
    ...
    def convert_vel_to_action(self, vel, obs_hist, env_ids=None):
        if self.random_init:
            clipped_vel = torch.clip(
                vel.to(self.device),
                min=torch.tensor([-3, -3, -np.pi]).to(self.device),
                max=torch.tensor([3, 3, np.pi]).to(self.device),
            )
            vel = clipped_vel
        else:
            vel = torch.clip(
    ...
    def compute_observations(self):
        ...
        yaw_orientations = torch.tensor(
            R.from_quat(orientations).as_euler("zyx")[:, 0]
        ).to(self.device)
        ...
        if self.custom_origins:
            if self.random_init:
                ...
                self.root_states[robot_env_ids, 1:2] = torch_rand_float(
                    -1.5, -0.7, (len(robot_env_ids), 1), device=self.device
                )
            else:
