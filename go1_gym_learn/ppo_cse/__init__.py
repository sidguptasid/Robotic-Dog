        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                ...
                for i in range(self.num_steps_per_env):
                    ...
                    if self.isTorque:
                        ...
                        vel_actions_train = self.alg.act(
                            obs_vel[:num_train_envs], obs_history_vel[:num_train_envs]
                        )
                        actions_train = self.env.convert_vel_to_action(
                            vel_actions_train,
                            obs_history[:num_train_envs],
                            env_ids=list(range(num_train_envs)),
                        )

                        vel_actions_eval = self.alg.actor_critic.act_student(
                            obs_history_vel[num_train_envs:]
                        )
                        actions_eval = self.env.convert_vel_to_action(
                            vel_actions_eval,
                            obs_history[num_train_envs:],
                            env_ids=list(range(num_train_envs, self.env.num_envs)),
                        )
            ...
            if it == 0:
                with open(os.path.join(model_root_path, "success_rate.txt"), "a") as f:
                    f.write(
                        f"Success Rate,\t\tEpisode Reward,\tGoal Distance Rew,\t\tWall Dist Rew,\t\tGoal Rew\n"
                    )
            ...
            if (it + 1) % 30:
                with open(os.path.join(model_root_path, "success_rate.txt"), "a") as f:
                    for (
                        ...
                        f.write(
                            f"{env_dones},\t\t{total_rew},\t\t{y_dist_rew},\t\t{wall_dist_rew},\t\t{ep_goal_rew}, \t\t{num_traj}\n"
                        )
                total_dones = []
                total_rews_buf = []
                y_dist_rews_buf = []
